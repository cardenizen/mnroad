...
public class TcQuery {
  ...
  static def dbConfig
  static def config
  static String tnbase = "TC_VALUES"
...
//      dbConfig = new ConfigSlurper(om.env).parse(cf.toURL())
//    } catch (java.io.FileNotFoundException fnfe) {
//      println "Cannot find configuration file: TcQueryParams.groovy"
//      return
//    }
    TcQuery dbq = new TcQuery()
//    dbq.initiate(dbConfig.db.server, dbConfig.db.user, dbConfig.db.pw)
//    if (!sql) {
//      println "Unable to connect to the database."
//      System.exit(1)
//    }
	def dataSourceFile = "grails-app/conf/DataSource.groovy"
	def cf = new File(dataSourceFile)
	if (!cf.exists()) {
		println "${dataSourceFile} does not exist"
//	  return
	}
	dataSourceFile = "../grails-app/conf/DataSource.groovy"
	cf = new File(dataSourceFile)
	def cfu = cf.toURL()
	if (cfu) {
	    cfu = cf.toURL()
	    dbConfig = new ConfigSlurper("development").parse(cfu)
	    sql = Sql.newInstance(dbConfig.dataSource.url as String,
	                dbConfig.dataSource.username as String,
	                dbConfig.dataSource.password as String,
	                dbConfig.dataSource.driverClassName as String)
	    def msg = "Connected to the ${om.env} environment."

			def	configFile = "../grails-app/conf/Config.groovy"
			cf = new File(configFile)
	    cfu = cf.toURL()
	    config = new ConfigSlurper("development").parse(cfu)
	    //println config
  ...
  int getData(int beginCell, int beginYear) {
    ...
    def cl = getCells()
    if (cl.keySet().size() == 0) {
    	println "cl.keySet().size() == 0"
      return tot
      }
    def tns = tcTableNames()
    if (tns.size() == 0) {
    	println "tcTableNames().size() == 0"
      return tot
    }
...
// Iterate over years
      for (Integer i=0; i < loy.size(); i++) {
      	...
      	if (beginYear <= yr) {
	        ...
	        if (hasTable(tableName)) {
	          ...
	          if (qm["dataQuery"]) {
	            def fileName = "${ds}/${yr}Cell${cellNumber}_${cellId}_TC.csv"
  ...
  def tcTableNames() {
    def rc = []
    String q = "select table_name from all_tables where owner='MNR' and table_name like '${tnbase}%'".toString()
    println q
    def ans = sql.rows(q)
    ans.each { row ->
      rc << row.TABLE_NAME
  ...
  boolean hasTable(String tableName) {
    boolean b = false
    def ans = sql.firstRow("select count(*) num from all_tables where owner='MNR' and table_name=?", [tableName])
