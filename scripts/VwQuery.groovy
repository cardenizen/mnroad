...
public class VwQuery {

  static def  sql
  static def config
  static String tnbase = "VW_VALUES"

  static main (args) {
    // Command line:
    // Oracle JDBC driver named ojdbc6.jar must be in the current directory
		// set JAVA_OPTS=-Xms128m -Xmx512m
		// groovy VwQuery.groovy -e prod
    Integer beginCell = 0
    Integer beginYear = 0
    int tot = 0
      ...
      for (Integer i=0; i < loy.size(); i++) {
      	...
      	if (beginYear <= yr) {
	        ...
	        if (hasTable(tableName)) {
	          ...
	          if (qm["dataQuery"]) {
	            def fileName = "${ds}/${yr}Cell${cellNumber}_${cellId}_VW.csv"
