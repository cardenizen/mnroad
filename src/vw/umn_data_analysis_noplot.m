...
clc;
diaryFilename = strcat( datestr(now,'mmmdd'), '_umnDiary.txt' );
%Store the diary in the output folder
filename = strcat(varargin{5},'/');
filename = strcat(filename,diaryFilename);
diary( filename );
ver;            % Get complete MATLAB version information
%memory;         % Get detailed information on the available memory
...
if nargin ~= 5 && nargin ~= 6
    ...
    disp('Parameter 6: Number of standard deviations (default is 3)');
...
if(single_file == 0)
    ...
    if(isempty(fxv))
        disp('The XV file is missing');
        disp('Try again with a valid file');
        cd(curpath);
        return;
    end
    cd(curpath);
                ...
                while(interval_complete == 0)
                    ...
                    if(sensor_type == VW)
                        if(val_end <= length(numcln_t))
                            ...
                            disp(sprintf('No temperature data - %s',char(sensor_title(col))));
...
%                   output(normal,8) = {1};
                ...
                disp(sprintf('No outliers found - %s',char(sensor_title(col))));
            end
        else
            disp(sprintf('All points out of range - %s',char(sensor_title(col))));
