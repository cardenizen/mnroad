...
//            ,"memberOf"
  ...
  /**
   * Given a user name, uses the Ad (Active Directory) class look up user info.
   *
   * If an entry is not found redirects to create.
   * If an entry is found returns
   * 1) List of roles from AD
   * 2) List of role from the DB
   * 3) List of attributes from AD
   */
    def lookupUser = {
      ...
      render(view:'create',model:[jsecUserInstance:jsecUserInstance
              ,roles:adRoles,dbRoles:dbRoles, attrs:displayList])
    }
